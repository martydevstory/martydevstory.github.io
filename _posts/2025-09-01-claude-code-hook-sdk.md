...
프로젝트 살펴보기 전에 `훅`에 대해서 좀 더 알아보겠습니다.
...
현재 설정에서 사용할 수 있는 도구를 정확히 확인하려면 Claude에게 직접 목록을 요청할 수 있습니다. 사용자 정의 MCP 서버를 추가하면 사용할 수 있는 도구가 변경될 수 있으므로 이는 특히 유용합니다.

### 3. 2.   3   도구 호출 데이터 구조 : 도구 호출을 받을 명령어 작성
...
### 3. 2.   4   Exit Code(종료 코드)와 제어 흐름 : 필요시 Claude에게 피드백 제공
...
- 투명한 작업 : Claude가 작업이 실패한 원인을 이해
- 유연한 매칭 : 여러 도구(Read, Grep 등)와 함께 작동
- 명확한 피드백 : 의미 있는 오류 메시지 제공

이 특정 예시는 .env 파일에 초점을 맞추고 있지만, 동일한 패턴으로 프로젝트의 모든 민감한 파일이나 디렉터리를 보호할 수 있습니다. 
...
### 3.  4. 1   Claude Code에서 훅 보안과 관련된 몇 가지 권장 사항
...
샘플 프로젝트와 다른 디렉터리에 위치할 수 있기 때문입니다.
...
`훅`은 AI를 통한 개발 특히, 대규모 프로젝트에서 발생하는 문제점을 해결하는 데 도움이 됩니다.
...
우선 `createSchema 함수`를 살펴보면 다음과 같이 db 매개 변수만 정의되어 있습니다. 
...
- 두 번째 인스턴스에 변경 사항을 검토하고 유사한 기존 쿼리를 확인하도록 요청
...
그러나 다음과 같이 `task.md`에 있는 프롬프트처럼 `슬랙 통합`과 함께 조금 복잡하게 구성하면 `기존 쿼리를 재사용하지 않고` 신규 쿼리를 생성하게 됩니다.
```
...
두 `훅` 모두 `pre-tool-use` 또는 `post-tool-use` `훅` 시스템을 사용합니다. TypeScript `훅`은 상대적으로 가볍고 빠르게 실행됩니다. 그러나 쿼리 중복 `훅`은 검토마다 별도의 Claude 인스턴스를 실행하므로 더 많은 리소스가 필요합니다.

쿼리 훅의 경우 다음과 같은 트레이드오프를 고려해야 합니다:

- `장점`: 중복이 적은 더 깔끔한 코드베이스
- `비용`: 각 쿼리 디렉터리 편집에 대한 추가 시간 및 API 사용량
- `권장사항`: 오버헤드를 최소화하기 위해 `중요한 디렉터리만` 모니터링

`훅`들은 Claude의 TypeScript SDK를 사용하여 AI와 프로그래밍 방식으로 상호작용을 합니다. 이를 통해 한 Claude 인스턴스가 다른 인스턴스의 작업을 검토하고 피드백을 제공할 수 있는 정교한 워크플로를 만들 수 있습니다.
...
앞서 살펴본 `PreToolUse` 및 `PostToolUse` `훅` 외에도 더 많은 `훅`이 있습니다:
...
그러나 다음처럼 우려스러운 부분이 있습니다.

- 사용자 명령어를 Claude Code 판단 후, `훅`을 실행할 때마다 관련 상황에 맞는 JSON 데이터를 `stdin`으로 전달하는데, 이때 `훅`의 유형(`PreToolUse`, `PostToolUse`, `Notification` 등)에 따라 `구조`가 변경됩니다.
- 해당 JSON 데이터에 포함된 `tool_input`은 호출된 도구(`PreToolUse` 및 `PostToolUse` `훅`의 경우)에 따라 `구조`가 달라집니다.
...
- 동일한 디렉터리의 Claude Code 인스턴스에서 모든 설정 상속
- 기본적으로 읽기 전용 권한
...
### 3.  7. 3   권한 및 도구
기본적으로 SDK는 읽기 전용 권한만 가집니다. 파일을 읽고, 디렉터리를 검색하고, grep 작업을 수행할 수 있지만 파일을 쓰거나 편집하거나 생성할 수는 없습니다.
...
또는 프로젝트 전체 액세스를 위해 `.claude` 디렉터리 내의 설정 파일에서 권한을 구성할 수 있습니다.
