---
title: n8n 소개
date: 2025-09-08 11:56:43 +/-TTTT
description : n8n
tags: [n8n, automate, python, llm, generative-ai, workflow]
math: true
toc: true
pin: false
image:
    path: assets/drafts/n8n/logo.png
    alt:
is_series: false
series_title: "n8n"
series_order: 1
...
### n8n 이란
[n8n](https://docs.n8n.io/)은 AI 기능과 비즈니스 프로세스 자동화를 결합한 공정-코드([Fair-code](https://faircode.io/)) 라이선스 워크플로 자동화 도구입니다.
`n8n`을 사용하면 모든 앱을 API와 연결하고, 코드를 거의 또는 전혀 사용하지 않고 해당 데이터를 조작할 수 있습니다.

다음과 같은 주요 기능을 제공합니다.
- 사용자 정의 가능: 매우 유연한 워크플로와 사용자 정의 노드를 구축하는 옵션을 제공합니다.
- 편리함: npm이나 Docker를 사용하여 n8n을 쉽게 사용 가능하고, 엔터프라이즈 버전을 통하여 호스팅 가능합니다.
...
- [n8n Cloud](https://docs.n8n.io/choose-n8n/#:~:text=to%20use%20it%3A-,n8n%20Cloud,-%3A%20hosted%20solution%2C%20no) : 호스팅 솔루션으로 아무것도 설치할 필요가 없습니다.
- [셀프 호스트](https://docs.n8n.io/choose-n8n/#:~:text=to%20install%20anything.-,Self%2Dhost,-%3A%20recommended%20method%20for) : 프로덕션이나 맞춤형 사용 사례에 권장되는 방법입니다.
	- [npm](https://docs.n8n.io/choose-n8n/#:~:text=customized%20use%20cases.-,npm,-Docker)
	- [도커](https://docs.n8n.io/hosting/installation/docker/)
	- [인기 플랫폼을 위한 서버 설정 가이드](https://docs.n8n.io/hosting/installation/server-setups/)
- [임베드](https://docs.n8n.io/embed/) : n8n 임베드를 사용하면 n8n을 화이트 라벨로 등록하여 자체 제품에 적용할 수 있습니다. 가격 및 지원은 임베드 웹사이트 에서 n8n에 확인 가능합니다.
...
### 사전 필수 조건
먼저 [Docker Desktop](https://docs.docker.com/get-docker/)을 설치합니다.
...
### Docker Compose 파일을 사용하여 실행

[Docker Compose with Postgresql](https://github.com/n8n-io/n8n-hosting/tree/main/docker-compose/withPostgres)을 다운로드하여 .env 파일에 값을 입력합니다.
...
![주요 인터페이스](../assets/drafts/n8n/interface.png)
_주요 인터페이스_
...
#### 상단 바
편집기 UI 의 상단 표시줄 에는 다음 정보가 포함되어 있습니다.
...
#### 캔버스
캔버스 는 편집기 UI의 회색 점선 그리드 배경입니다. 여러 아이콘과 다양한 기능을 가진 노드가 표시됩니다.
...
- 앱 또는 액션 노드는 데이터를 추가, 삭제, 편집하고, 외부 데이터를 요청 및 전송하며, 다른 시스템에서 이벤트를 트리거합니다. 이러한 노드의 전체 목록은 액션 노드 라이브러리 를 참조하세요 .
- 트리거 노드는 워크플로를 시작하고 초기 데이터를 제공합니다. 트리거 노드 목록은 트리거 노드 라이브러리 를 참조하세요.
...
- 클러스터 노드 는 주로 AI 워크플로에서 기능을 제공하기 위해 함께 작동하는 노드 그룹입니다. 자세한 내용은 클러스터 노드를 참조하세요.

#### 노드 찾기
편집기 UI 오른쪽의 노드 패널 에서 사용 가능한 모든 노드를 확인할 수 있습니다. 

노드 패널을 여는 방법은 세 가지가 있습니다.

- 캔버스 오른쪽 상단에 있는 `+` 아이콘을 클립합니다.
- 캔버스에서 기존 노드(다른 노드를 추가하려는 노드)의 오른쪽에 있는 `+` 아이콘을 클릭합니다.
- `Tab`키를 클릭하세요.

노드 패널에서 첫 번째 노드를 추가할 때 다양한 트리거 노드 범주가 표시됩니다. 트리거 노드를 추가하면 노드 패널이 `Advanced AI`, `Actions in an App`, `Data transformation`, `Flow`, `Core`, 그리고 `Human in the loop`로 변경됩니다.

특정 노드를 찾으려면 노드 패널 상단에 있는 검색 입력을 사용하세요.

![기존 노드 + 아이콘 클릭하여 노드 찾기](../assets/drafts/n8n/search-node.png)
...
#### 노드 버튼 기능들
노드에 마우스를 올려 놓으면 맨 위에 세 개의 아이콘이 나타납니다.
...
![노드 버튼 기능](../assets/drafts/n8n/node-buttons.png)
_노드 버튼 기능_

### 참조

- 공식문서 :  [링크](https://docs.n8n.io/hosting/installation/docker/)
- 위키피디아 : [링크](https://wikidocs.net/290939)
- Self hosted AI Starter Kit : [링크](https://github.com/n8n-io/self-hosted-ai-starter-kit)
- n8n 단점 - reddit : [링크](https://www.reddit.com/r/n8n/comments/1ketr7h/real_n8n_and_nocode_pain_points_no_one_talks_about/?tl=ko)